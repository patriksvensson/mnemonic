...
public static class MnemonicGenerator
{
    ...
    /// <returns>The generated mnmemonic.</returns>
    public static string Generate(MnemonicSettings? settings = null)
    {
        settings ??= MnemonicSettings.Default;

        var leftIndex = Random.Shared.Next(0, _left.Length);
        var rightIndex = Random.Shared.Next(0, _right.Length);
        return _left[leftIndex] + settings.Separator + _right[rightIndex];
    }

    // Borrowed from the Moby project, licensed under Apache-2.0:
    ...
    // Borrowed from the Moby project, licensed under Apache-2.0:
    // https://github.com/moby/moby/blob/39f7b2b6d0156811d9683c6cb0743118ae516a11/pkg/namesgenerator/names-generator.go#L852-L863
    private static readonly string[] _right =
    [
